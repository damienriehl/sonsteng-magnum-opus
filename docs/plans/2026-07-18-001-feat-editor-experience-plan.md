...
| 6 | Review + digest | Token-gated `/edit/review` (Accept/Decline, drift resolution, sibling grouping) + **cumulative daily digest** (all `pending` regardless of age; delivered only when non-empty) via a scheduled agent session; ntfy optional later. |
...
- **P5 — Digest cron (scheduled agent), docs (README + API contracts + editor guide for John — one printable page), UAT (E1–E18 driven literally on Windows-viewport + iPad-viewport + one real-device pass when available), red-team the token surfaces, deploy DEV.**
