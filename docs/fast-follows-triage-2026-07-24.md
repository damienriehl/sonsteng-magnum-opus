...
|---|------|------------------------|-----------------|--------|----------------|
| 1 | **PROD injector wiring** (`EDIT_UPSTREAM` → Pages) | **Built, not flipped.** `wrangler.jsonc` `env.production` points `EDIT_UPSTREAM` at `https://legalpracticum.org/platform/` and `EDIT_ORIGIN` at `sonsteng-chat-production…`; the current Publisher release runbook is in `docs/prod-release-operations.md`. | Yes — but it remains a supervised production-release gate, not routine engineering work. | S (config + one release) | **NEXT** — after the protected canary and recovery proof. |
