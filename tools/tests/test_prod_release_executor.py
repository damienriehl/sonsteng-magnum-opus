...
def test_wrangler_adapters_pin_candidate_root_and_timeout(tmp_path):
    ...
    class RollbackResponse:
        def __enter__(self): return self
        def __exit__(self, *_): pass
        def read(self): return json.dumps({"success":True,"result":{"id":"pagesdeploy123"}}).encode()
    rollback_requests = []
    def rollback_opener(request, timeout):
        rollback_requests.append((request, timeout))
        return RollbackResponse()
    WranglerPagesAdapter("sonsteng",site,"https://pages.example",root,
                         run=run,timeout=17,account_id="account123",
                         api_token="pages-token",opener=rollback_opener).restore("pagesdeploy123")
    WranglerWorkerAdapter(config,"https://worker.example",root,
                          run=run,timeout=19).restore(
                              "12345678-1234-4234-8234-123456789abc")
    assert [call[1]["timeout"] for call in calls] == [17, 19, 19, 19]
    ...
    request, timeout = rollback_requests[0]
    assert request.full_url.endswith("/accounts/account123/pages/projects/sonsteng/deployments/pagesdeploy123/rollback")
    assert request.method == "POST" and json.loads(request.data) == {}
    assert request.get_header("Authorization") == "Bearer pages-token" and timeout == 17
    assert calls[3][0][2:5] == ["versions","deploy",
      "12345678-1234-4234-8234-123456789abc"]
