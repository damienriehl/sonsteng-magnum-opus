...
const ALLOWED = parseAllowedOrigins("https://sonsteng-dev.damienriehl.com,https://legalpracticum.org");
...
test("OPTIONS preflight returns 204 + CORS for allowed, 403 bare for others", () => {
  const ok = handlePreflight(req("https://legalpracticum.org", "OPTIONS"), ALLOWED);
  assert.equal(ok.status, 204);
  assert.equal(ok.headers.get("Access-Control-Allow-Origin"), "https://legalpracticum.org");
